  ...
  for (i = 1; i < numcli; i++) {
   ...
   printf("produciendo %d\n",i);
    ...
    if (pid == 0 ) {
      ...
      printf("produciendo hijo\n");

      //Obtenemos el pid del hijo
      mipid = getpid();

       printf("obtenido  pid %d\n", mipid);


      //Obttenemos el FIFO del servidor
      mififo = obtenerfifo(dfifoe, dfifos, mipid);


       printf("obtenido  fifo\n");

     

      //Producimos y pintamos el mensaje
      producir(c, mififo, mipid);

      //Salimos del subproceso
      exit(EXIT_SUCCESS);

    }else {
      //Padre

      //Incrementamos el contador de caracter
      c++;
